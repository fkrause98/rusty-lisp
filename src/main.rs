...
pub fn string<'a>() -> Parser<'a, u8, LispVal> {
    ...
    let string_matcher = one_of(b"\"") * ((none_of(b"\\\"") | escaped_seq).repeat(0..))
      - one_of(b"\"")
        .collect();
        string_matcher.convert(|p| String::from_utf8(p)).map(|s| LispVal::String(s))
}
...
pub fn binary_number<'a>() -> Parser<'a, u8, LispVal> {
    ...
    (prefix.discard() * (one_of(b"01").repeat(1..))).collect()
                                        .map(|parsed| {
...
pub fn octal_number<'a>() -> Parser<'a, u8, LispVal> {
    ...
    (prefix.discard() * (one_of(b"01234567").repeat(1..))).collect()
                                        .map(|parsed| {
        let as_string = String::from_utf8_lossy(&parsed[2..]);
...
pub fn hex_number<'a>() -> Parser<'a, u8, LispVal> {
    ...
    (prefix.discard() * (one_of(b"0123456789abcdefABCDEF").repeat(1..))).collect()
                                        .map(|parsed| {
...
mod test {
    ...
    fn read_string_with_quote() {
        assert_eq!(read_expr(b"\"1\\\"23\""), LispVal::String("1\"23".to_owned()));
    }
}
